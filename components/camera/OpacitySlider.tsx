...
export function OpacitySlider({
  ...
  return (
    ...
    >
      <View
        className={`${compact ? "mb-1" : "mb-2"} flex-row items-center justify-between`}
      >
        <View className="flex-row items-center gap-2">
          <Ghost
            size={compact ? 16 : 18}
            color={disabled ? theme.mutedText : theme.accent}
          />
          <Text className={`font-inter-medium text-white ${labelCls}`}>
            Ghost strength
          </Text>
        </View>
        <Text
