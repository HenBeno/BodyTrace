...
export function CameraControlSheet({
  ...
  const advancedBlock = advancedOpen ? (
    ...
    >
      ...
      <View className="mb-3 flex-row items-center justify-between rounded-2xl border border-white/10 bg-white/5 px-3 py-2.5">
        <View className="mr-2 min-w-0 flex-1 flex-row items-center gap-2">
          ...
          <View className="min-w-0 flex-1">
            ...
            <Text
              className="font-inter-medium text-[11px] text-white/55"
              numberOfLines={2}
            >
      ...
      <Text className="mb-1.5 font-inter-semibold text-[10px] uppercase tracking-[0.12em] text-accent/95">
        Reference
      </Text>
      <ScrollView
        horizontal
      ...
      >
        ...
        >
          <Text className="font-inter-medium text-[11px] text-white">
            Oldest
          </Text>
        </Pressable>
  ...
  const inner = (
    ...
    >
      ...
      {progressLabel ? (
        <Text className="mb-1.5 text-center font-inter-medium text-[11px] text-white/55">
          {progressLabel}
        </Text>
      ) : null}
        ...
        <View className="mb-2 flex-row flex-wrap justify-center gap-1.5">
          {photoOrder.map((ang, i) => {
            ...
            return (
              ...
              >
                <Text
                  className={`font-inter-semibold text-[11px] capitalize ${active ? "text-canvas" : "text-white/75"}`}
                >
                  {ang}
                  {done ? " ✓" : ""}
                </Text>
      ...
      <View className={advancedOpen ? "border-t border-white/10 pt-2" : ""}>
        {footer}
      </View>
    </View>
  );
  ...
  return (
    <BlurView
      ...
      className="overflow-hidden rounded-t-2xl border-t border-white/10"
    >
      {sheetBody}
    </BlurView>
  );
