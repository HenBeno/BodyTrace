...
export function CameraControlSheet({
  ...
  const advancedScrollMax = useMemo(() => {
    ...
    const reserved =
      footerReserve + compactReserve + progressReserve + chipReserve + bottomPad + chromeMargin;
  ...
  const advancedBlock = advancedOpen ? (
    <ScrollView
      ...
      {title ? (
        <Text className="mb-2 font-inter-semibold text-xs uppercase tracking-wide text-white/70">{title}</Text>
      ) : null}
      ...
      <View className="mb-3 flex-row items-center justify-between rounded-2xl border border-white/10 bg-white/5 px-3 py-2.5">
        <View className="mr-2 min-w-0 flex-1 flex-row items-center gap-2">
          <Ghost size={18} color={ghostEnabled ? theme.accent : theme.mutedText} />
          <View className="min-w-0 flex-1">
            <Text className="font-inter-semibold text-white" numberOfLines={1}>
              Match previous photo
            </Text>
            <Text className="text-[11px] font-inter-medium text-white/55" numberOfLines={2}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} className="pb-1">
        <Pressable
          ...
          className={`mr-2 rounded-xl border px-2.5 py-1.5 ${
            referenceEntryId === null ? 'border-accent bg-accent-dim' : 'border-white/20 bg-white/5'
          }`}>
          <Text className="text-[11px] font-inter-medium text-white">Oldest</Text>
        </Pressable>
        {referenceEntries.map((e) => (
          <Pressable
            ...
            className={`mr-2 rounded-xl border px-2.5 py-1.5 ${
              referenceEntryId === e.id ? 'border-accent bg-accent-dim' : 'border-white/20 bg-white/5'
            }`}>
  ...
  const inner = (
    <View className="border-t border-white/10 px-3 pt-2" style={{ paddingBottom: bottomPad }}>
      {advancedBlock}

      {progressLabel ? (
        <Text className="mb-1.5 text-center text-[11px] font-inter-medium text-white/55">{progressLabel}</Text>
      ) : null}
      ...
      <View className="mb-2 flex-row items-center gap-2">
        <Pressable
          ...
          accessibilityLabel={showGuide ? 'Hide alignment grid' : 'Show alignment grid'}
          hitSlop={10}
        ...
        <Pressable
          ...
          accessibilityRole="button"
          accessibilityLabel={advancedOpen ? 'Hide ghost and reference' : 'Show ghost and reference'}
          hitSlop={8}
          className={`h-10 w-10 items-center justify-center rounded-xl active:opacity-80 ${
            advancedOpen ? 'bg-accent/30' : 'bg-white/10'
          }`}>
          <SlidersHorizontal size={18} color={advancedOpen ? '#fff' : theme.accent} />
        </Pressable>
      </View>
      ...
      <View className={advancedOpen ? 'border-t border-white/10 pt-2' : ''}>{footer}</View>
    </View>
  );

  const sheetBody = <View style={{ backgroundColor: 'rgba(8,10,13,0.62)' }}>{inner}</View>;

  if (Platform.OS === 'web') {
    return <View className="border-t border-white/10 bg-surface/95">{inner}</View>;
  }
