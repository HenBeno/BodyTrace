...
export function SliderOverlay({ beforeUri, afterUri }: SliderOverlayProps) {
  ...
  const pan = Gesture.Pan()
    ...
    .onUpdate((e) => {
      ...
      split.value = Math.max(0.05, Math.min(0.95, startSplit.value + e.translationX / w));
    })
    .onEnd(() => {
      split.value = withSpring(split.value, { damping: 18, stiffness: 220, mass: 0.35 });
  ...
  return (
    <GestureDetector gesture={pan}>
      <View
        ...
        <ResolvedExpoImage uri={afterUri} style={StyleSheet.absoluteFillObject} contentFit="cover" />
        ...
        <View pointerEvents="none" className="absolute left-3 right-3 top-3 flex-row justify-between">
          <View className="rounded-full border border-white/15 bg-canvas/55 px-2.5 py-1">
            <Text className="text-[10px] font-inter-semibold uppercase tracking-[0.12em] text-white/90">Before</Text>
          </View>
          <View className="rounded-full border border-white/15 bg-canvas/55 px-2.5 py-1">
            <Text className="text-[10px] font-inter-semibold uppercase tracking-[0.12em] text-white/90">After</Text>
          </View>
        </View>
