...
function measurementChips(entry: Entry): { label: string; value: string }[] {
  ...
  if (waist) chips.push({ label: 'Waist', value: formatMeasurementDisplay(waist) });
  if (weight) chips.push({ label: 'Weight', value: formatMeasurementDisplay(weight) });
        ...
        <Animated.View entering={FadeIn.duration(170)}>
          <View className="mb-6 flex-row">
            <View className="w-8 items-center pt-1">
              {index > 0 ? <View className="mb-1 h-4 w-[2px] rounded-full bg-accent/45" /> : <View className="h-4" />}
              <View
            ...
            <View className="min-w-0 flex-1 pl-1">
              <View className="overflow-hidden rounded-3xl border border-white/10 bg-black shadow-vault">
                <View className="relative w-full" style={{ aspectRatio: 3 / 4 }}>
                  <ResolvedExpoImage
                    uri={heroUri}
                  ...
                  <View className="absolute bottom-0 left-0 right-0 px-4 pb-4 pt-8">
                    <Text className="text-[11px] font-inter-medium uppercase tracking-[0.14em] text-white/70">
                      {formatDate(entry.createdAt)}
                    </Text>
                    <View className="mt-1 flex-row items-center justify-between gap-2">
                      <Text className="flex-1 font-inter-bold text-xl text-white" numberOfLines={1}>
                        Progress check-in
                      </Text>
                    ...
                    <View className="mt-3 flex-row flex-wrap gap-2">
                      {summaryEmpty ? (
                        <View className="rounded-full border border-white/25 bg-white/10 px-3 py-1">
                          <Text className="text-xs font-inter-medium text-white/90">No measurements</Text>
                        </View>
                      ) : (
                        chips.map((c) => (
                          <View
                            ...
                            <Text
                              className="text-sm font-inter-semibold text-accent"
