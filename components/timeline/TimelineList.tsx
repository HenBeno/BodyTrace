    ...
    <View className="w-full px-1 pb-6 pt-2">
      <View className="mx-auto w-full max-w-sm items-center rounded-3xl border-2 border-dashed border-slate-300 bg-white px-6 py-10 dark:border-white/25 dark:bg-elevated">
        <Text className="text-center text-base leading-6 text-slate-900 dark:text-vault-fg">
          No entries yet. Use New entry to capture front, side, and back photos — that is the only way they are saved to
          your timeline. Everything stays private, on-device.
        </Text>
        <Link href="/entry/new" asChild>
          <Pressable
            ...
            <Text className="text-center font-inter-semibold text-base text-canvas">New entry</Text>
          </Pressable>
        </Link>
      </View>
    </View>
...
export function TimelineList({ entries, ListHeaderComponent }: TimelineListProps) {
  const renderItem: ListRenderItem<Entry> = ({ item, index }) => (
    <TimelineEntry entry={item} index={index} isLast={index === entries.length - 1} />
  );
