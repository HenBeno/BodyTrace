...
export function SegmentedControl<T extends string>({
  ...
  const minH =
    density === 'micro' ? 'min-h-[30px]' : density === 'compact' ? 'min-h-[36px]' : 'min-h-[44px]';
  const textSize = density === 'micro' ? 'text-[11px]' : density === 'compact' ? 'text-xs' : 'text-sm';
      ...
      {options.map((o) => {
        ...
        return (
          <Pressable
            ...
            <Text className={`text-center font-inter-semibold ${textSize} ${active ? activeText : inactiveText}`}>
              {o.label}
            </Text>
          </Pressable>
        );
