...
export function TrustBadge({ caption = 'Private — stored only on this device' }: TrustBadgeProps) {
  return (
    <View className="flex-row items-center gap-2 rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2 dark:border-white/10 dark:bg-elevated/90">
      ...
      <Text className="flex-1 text-xs font-inter-medium leading-5 text-slate-600 dark:text-vault-muted">{caption}</Text>
    </View>
  );
}
