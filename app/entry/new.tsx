...
export default function NewEntryScreen() {
    ...
    const [captured, setCaptured] = useState<
        Partial<Record<PhotoAngle, string>>
    >({});
    const [referenceEntryId, setReferenceEntryId] = useState<string | null>(
        null,
    );
    ...
    const onCapture = useCallback(async () => {
        ...
        try {
            ...
            if (!photo?.uri) {
                Alert.alert(
                    "Capture failed",
                    "No image was returned from the camera.",
                );
                return;
            }
                ...
                >
                    ...
                    >
                        <Text className="mb-5 text-sm leading-6 text-slate-600 dark:text-vault-muted">
                            Type a value, then pick cm or in (kg or lb for
                            weight) on the right — or leave blank. Only filled
                            rows are saved, in the units you chose.
                        </Text>
                        {MEASUREMENT_GROUPS.map((group) => (
                            <Card key={group.title} className="mb-4">
                                ...
                                {group.keys.map((key) => {
                                    const label =
                                        MEASUREMENT_LABELS.find(
                                            (m) => m.key === key,
                                        )?.label ?? key;
                                    const unitVal =
                                        measurementUnits[key] ??
                                        defaultUnitForKey(key);
                                    return (
                                        <View
                                            key={key}
                ...
                >
                    <View className="flex-1 items-center justify-center">
                        ...
                        <Text className="mt-2 max-w-sm text-center text-slate-600 dark:text-vault-muted">
                            New entries need the camera. Open BodyTrace in Expo
                            Go on iOS or Android, or use a development build on
                            a device.
                        </Text>
                    </View>
                </SafeAreaView>
                ...
                >
                    <View className="flex-1 items-center justify-center">
                        <Text className="mb-4 text-center text-base text-white">
                            Camera access is needed to capture your front, side,
                            and back photos.
                        </Text>
                        <Button
                            title="Allow camera"
            ...
            <View className="flex-1 bg-black">
                ...
                >
                    <CameraControlSheet
                        ...
                        angle={currentAngle}
                        onAngleChange={(a) =>
                            setPhotoIndex(PHOTO_ORDER.indexOf(a))
                        }
