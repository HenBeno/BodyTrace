...
import { BodyMeasurementZoneIcon, NotesGlyphIcon } from '@/components/measurements/BodyMeasurementIcons';
...
export default function NewEntryScreen() {
  ...
  >(() => {
    const init = {} as Record<keyof Entry['measurements'], CircumferenceUnit | WeightUnit>;
  ...
  const onSkipCurrentAngle = useCallback(() => {
    ...
    if (capturedCount === 0) {
      Alert.alert('Photo required', 'Capture at least one angle before continuing to measurements.');
      return;
    }
    setPhase('details');
  ...
  const onSave = useCallback(async () => {
    ...
    for (const { key } of MEASUREMENT_LABELS) {
      ...
      if (key === 'weight') {
        ...
        measurements[circKey] = { value: v, unit: unit as CircumferenceUnit };
      }
    }
    const id = newEntryId();
    const entry: Entry = {
  ...
  }, [addEntry, captured, capturedCount, measurementInputs, measurementUnits, notes]);
          ...
          options={{
            ...
            headerLeft: () => (
              <Pressable
                ...
                <Text className="font-inter-semibold text-base text-accent">Photos</Text>
              </Pressable>
            ),
            headerRight: () => (
              <Pressable accessibilityRole="button" onPress={() => safeGoBack()} className="py-2 pl-2">
                <Text className="font-inter-medium text-base text-slate-500 dark:text-vault-muted">Cancel</Text>
              </Pressable>
            ),
          }}
        />
        <KeyboardAvoidingView
          ...
          <ScrollView
            ...
            <Text className="mb-5 text-sm leading-6 text-slate-600 dark:text-vault-muted">
              Type a value, then pick cm or in (kg or lb for weight) on the right — or leave blank. Only filled rows are
              saved, in the units you chose.
            </Text>
            {MEASUREMENT_GROUPS.map((group) => (
              <Card key={group.title} className="mb-4">
                <Text className="mb-3 text-[11px] font-inter-semibold uppercase tracking-[0.14em] text-vault-muted">
                  {group.title}
                </Text>
                {group.keys.map((key) => {
                  const label = MEASUREMENT_LABELS.find((m) => m.key === key)?.label ?? key;
                  const unitVal = measurementUnits[key] ?? defaultUnitForKey(key);
                  return (
                    <View
                      key={key}
                      ...
                      <View className="h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-accent/12 dark:bg-white/10">
                      ...
                      <View className="shrink-0 flex-row items-center gap-1.5">
                        <TextInput
                          ...
                          onChangeText={(t) => setMeasurementInputs((prev) => ({ ...prev, [key]: t }))}
                        ...
                        {isWeightMeasurementKey(key) ? (
                          <SegmentedControl
                            ...
                            onChange={(id) => setMeasurementUnits((prev) => ({ ...prev, [key]: id }))}
                            density="micro"
            ...
            <Card className="mb-4">
              <View className="mb-2 flex-row items-center gap-2.5">
                <View className="h-10 w-10 items-center justify-center rounded-2xl bg-accent/12 dark:bg-white/10">
                  <NotesGlyphIcon
                    size={22}
                    accentColor={theme.accent}
                ...
                <Text className="font-inter-semibold text-lg text-slate-900 dark:text-vault-fg">Notes (optional)</Text>
              </View>
              <TextInput
                value={notes}
  ...
  if (Platform.OS === 'web') {
    return (
      <>
        ...
        <SafeAreaView className="flex-1 bg-slate-50 px-4 dark:bg-canvas" edges={['bottom']}>
          <View className="flex-1 items-center justify-center">
            ...
            <Text className="mt-2 max-w-sm text-center text-slate-600 dark:text-vault-muted">
              New entries need the camera. Open BodyTrace in Expo Go on iOS or Android, or use a development build on
              a device.
            </Text>
          </View>
        </SafeAreaView>
  ...
  if (!permission.granted) {
    return (
      <>
        <Stack.Screen options={{ title: 'New entry' }} />
        <SafeAreaView className="flex-1 bg-canvas px-4" edges={['top', 'bottom']}>
          <View className="flex-1 items-center justify-center">
            <Text className="mb-4 text-center text-base text-white">
              Camera access is needed to capture your front, side, and back photos.
            </Text>
      ...
      <View className="flex-1 bg-black">
        <View className="absolute inset-0">
          <CameraView
            ...
            enableTorch={false}
          />
          <GhostOverlay imageUri={ghostUri} opacity={ghostEnabled ? ghostOpacity : 0} />
          ...
          <View className="absolute left-0 right-0 top-12 items-center px-4">
            <View className="rounded-full border border-white/10 bg-canvas/60 px-4 py-2">
              <Text className="text-center font-inter-semibold capitalize text-white">{currentAngle}</Text>
              <Text className="text-center text-xs text-white/85">Match the ghost, then capture</Text>
            </View>
          </View>
        </View>
            ...
            <View className="mt-1">
              {captured[currentAngle] ? (
                <View className="gap-2">
                  <Text className="text-center text-xs font-inter-medium text-accent/95">
                    Photo saved for this angle.
                  </Text>
                  <View className="flex-row gap-2">
                    <View className="flex-1">
                      <Button title="Retake" variant="secondary" onPress={onRetakeCurrent} />
                    </View>
                    <View className="flex-1">
                      <Button
                        title={photoIndex < PHOTO_ORDER.length - 1 ? 'Next angle' : 'Measurements'}
                ...
                <View className="gap-2">
                  <Button
                    title={capturing ? 'Capturing…' : `Capture ${currentAngle}`}
                    loading={capturing}
                    onPress={onCapture}
                  />
                  <Button title="Skip this angle" variant="secondary" onPress={onSkipCurrentAngle} />
                </View>
              )}
            </View>
          }
