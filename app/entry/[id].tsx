...
export default function EntryDetailScreen() {
  ...
  if (!entry) {
    return (
      <SafeAreaView className="flex-1 items-center justify-center bg-slate-50 dark:bg-canvas">
        <Text className="font-inter-medium text-vault-muted">
          Entry not found.
        </Text>
      </SafeAreaView>
    );
    ...
    <>
      ...
      >
        ...
        >
              ...
              return (
                ...
                >
                  {uri ? (
                    ...
                    >
                      <Text className="px-2 text-center font-inter-medium text-xs text-slate-500 dark:text-vault-muted">
                        No {angle} photo
                      </Text>
                    </View>
                  )}
                  <Text className="bg-canvas/90 py-1.5 text-center font-inter-semibold text-[11px] uppercase tracking-[0.1em] text-white">
                    {angle}
                  </Text>
                </View>
              );
          ...
          <Card>
            <Text className="mb-4 font-inter-bold text-lg text-slate-900 dark:text-vault-fg">
              Measurements
            </Text>
            {statItems.length === 0 ? (
              <Text className="font-inter-medium text-vault-muted">
              ...
              <View className="flex-row flex-wrap gap-3">
                {statItems.map((row) => (
                  ...
                  >
                    <Text className="font-inter-semibold text-[11px] uppercase tracking-[0.12em] text-vault-muted">
                      {row.label}
                    </Text>
                    <Text
          ...
          {entry.notes ? (
            <Card className="mt-4">
              <Text className="mb-2 font-inter-bold text-lg text-slate-900 dark:text-vault-fg">
                Notes
              </Text>
              <Text className="leading-6 text-slate-600 dark:text-vault-muted">
                {entry.notes}
              </Text>
            </Card>
          ) : null}
