...
export default function CompareScreen() {
  ...
  if (entries.length < 2) {
    return (
      <SafeAreaView className="flex-1 bg-slate-50 px-4 dark:bg-canvas" edges={['top']}>
        <ScreenHeader
          title="Compare"
        ...
        <Card>
          <Text className="leading-6 text-slate-700 dark:text-vault-muted">
            Add at least two entries to your timeline to compare before and after.
          </Text>
        </Card>
      </SafeAreaView>
    );
      ...
      <ScrollView
        ...
        {older && newer ? (
          beforeUri && afterUri ? (
            <>
              ...
              {showShareRow ? (
                <View className="mt-4 flex-row gap-2">
                  <View className="flex-1">
                    <Button title="Share earlier" variant="secondary" onPress={onShareBefore} />
                  </View>
                  <View className="flex-1">
                    <Button title="Share later" variant="secondary" onPress={onShareAfter} />
                  </View>
                </View>
              ) : null}
              <View className="mt-3 flex-row items-center justify-center gap-2 opacity-80">
                ...
                <Text className="flex-1 text-center text-xs text-vault-muted">
                  Exports use the system share sheet (on-device files when saved locally). Tap ··· to share.
                </Text>
              </View>
            </>
          ) : (
            <Card>
              <Text className="leading-6 text-slate-700 dark:text-vault-muted">
                One or both entries have no photo for this angle. Pick another angle above, or add that photo in a new
                entry.
              </Text>
            </Card>
          )
