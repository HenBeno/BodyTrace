...
export default function SettingsScreen() {
    ...
    const adjustMonthlyDate = useCallback(
        async (delta: number) => {
            await runUpdate({ monthlyDate: clamp(settings.monthlyDate + delta, 1, 28) });
        },
    ...
    const adjustEveryXHours = useCallback(
        async (delta: number) => {
            await runUpdate({ everyXHours: clamp(settings.everyXHours + delta, 1, 23) });
        },
    ...
    const adjustCountPerDay = useCallback(
        async (delta: number) => {
            await runUpdate({ countPerDay: clamp(settings.countPerDay + delta, 1, 6) });
        },
            ...
            >
                ...
                <Card className="mb-4">
                    ...
                    <Text className="mt-3 text-sm leading-6 text-slate-600 dark:text-vault-muted">
                        When enabled, BodyTrace asks for Face ID, Touch ID, or
                        fingerprint after you leave the app and when you open it
                        again.
                    </Text>
